        ...
        for dir in dirs:
            ...
            if CHECK_FOLDER:
                ...
                if not apply_augmentation:
                    ...
                    print("Number of final videos for class {} : {}".format(dir,
                                                                            int(len(os.listdir(path_directory_save)))))
                    print("-----------------------------------------------")
                ...
                for sub_subdir, sub_dirs, sub_files in os.walk(path_subdir):
                    for name_file in sub_files:
                        if name_file.endswith('.mp4'):
                            ...
                            print("Take the .avi file: {} - Generate the .mp4 file {}: ".format(path_file_avi,
                                                                                                path_file_mp4))
...
def make_train_test_val_division(path_original_dataset, dir_path_ttv):
    ...
    for subdir, dirs, files in os.walk(path_original_dataset):
        for dir in dirs:
            ...
            print(
                "Number_files on directory '{}' with duration grater then 1 sec: {}".format(path_subdir, number_files))
            ...
            print("Number of files into dir '{}': {}".format(os.path.join(dir_path_ttv, "train", dir),
                                                             len(os.listdir(os.path.join(dir_path_ttv, "train", dir)))))
            ...
            for idx in test_index_list:
                ...
                # print("Copia immagine da '{}' a {} ".format(src_dir, dst_dir))
                shutil.copy2(path_file, dst_dir)
            print("Number of files into dir '{}': {}".format(os.path.join(dir_path_ttv, "test", dir),
                                                             len(os.listdir(os.path.join(dir_path_ttv, "test", dir)))))
            ...
            for idx in val_index_list:
                ...
                # print("Copia immagine da '{}' a {} ".format(src_dir, dst_dir))
                shutil.copy2(path_file, dst_dir)
            print("Number of files into dir '{}': {}".format(os.path.join(dir_path_ttv, "val", dir),
                                                             len(os.listdir(os.path.join(dir_path_ttv, "val", dir)))))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dir_dataset_grouped_ttv_mp4',
                        type=str,
                        default="../dataset/PPBEmo_video_data",
    ...
    # 1) Split the dataset into train, test and val subdir
    make_train_test_val_division(dir_dataset_grouped_ttv_mp4, dir_dataset_grouped_ttv_mp4)

    print("****************************************************")
    print("****************************************************")

    # 2) Create csv for train, val and test dataset
    class2label = create_dict_class2label(dir_dataset_grouped_ttv_mp4)
    ...
    df_train = create_csv(dir_dataset_grouped_ttv_mp4, "train", class2label)
    ...
    df_test = create_csv(dir_dataset_grouped_ttv_mp4, "test", class2label)
    ...
    df_val = create_csv(dir_dataset_grouped_ttv_mp4, "val", class2label)
