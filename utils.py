...
def create_augmented_video(frame_list, path_augmented_video, fps):
    writer = skvideo.io.FFmpegWriter(path_augmented_video,
                                     inputdict={'-r': str(fps)},
                                     outputdict={'-r': str(fps), '-c:v': 'libx264', '-preset': 'ultrafast',
                                                 '-pix_fmt': 'yuv444p'})
...
y_true = np.random.choice([0, 1], size=576)
y_predicted = [item if i > 28 else 0 for i, item in enumerate(y_true)]
mse = np.random.normal(loc=0.79, scale=1.0, size=576)
