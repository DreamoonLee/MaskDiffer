...
class ClassificationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_frame = pd.read_excel('./dataset/PPBEmo_Emotion_label.xlsx')
        label_frame.set_index("PPB_Emo_dataset@video-face-CRGB", inplace=True)
        all_intensity = label_frame.loc[:, "intensity"]
        mean = all_intensity.mean()
        std = all_intensity.std()

        video_name = os.path.basename(video_path)[:-6]
        try:
            dataframe_item = label_frame.loc[video_name]
        except:
            print("*****************")
            print(video_name)
            print(video_path)
        ...
        intensity = (intensity - mean) / std

        return inputs, label, (intensity, mean, std)
...
def create_loaders(df_dataset_train, df_dataset_val, df_dataset_test, cfg):
    ...
    classification_dataset_train = ClassificationDataset(df_dataset=df_dataset_train, cfg=cfg,
    ...
    if df_dataset_test is not None:
        classification_dataset_test = ClassificationDataset(df_dataset=df_dataset_test, cfg=cfg,
                                                            dataset_path=dataset_path)
